...
st.markdown(
    ...
    <style>
        ...
        .block-container { max-width: 1400px; padding-top: 2.0rem; padding-bottom: 3rem; }
        ...
        .note-box { background: white; border: 1px solid #e7e9ee; border-radius: 14px; padding: 14px 16px; color: #5d6574; font-size: 0.92rem; line-height: 1.65; }
    </style>
    """,
...
crime_map.update_layout(
    title=dict(text=f"{selected_crime} 발생률", x=0.02, xanchor="left", font=dict(size=17)),
    height=470,
    margin=dict(l=0, r=0, t=45, b=0),
    paper_bgcolor="white",
    plot_bgcolor="white",
    coloraxis_colorbar=dict(title="1만 명당", thickness=10, len=0.7, outlinewidth=0),
)
...
price_map.update_layout(
    title=dict(text="아파트 ㎡당 매매가격 중앙값", x=0.02, xanchor="left", font=dict(size=17)),
    height=470,
    margin=dict(l=0, r=0, t=45, b=0),
    paper_bgcolor="white",
    plot_bgcolor="white",
    coloraxis_colorbar=dict(title="만원/㎡", thickness=10, len=0.7, outlinewidth=0),
)
...
st.markdown(
    ...
    <div class="note-box">
    <b>해석 시 유의사항</b><br>
    범죄율은 주민등록인구 1만 명당 발생건수로 계산했습니다.
    따라서 유동인구가 많은 도심·상업지역의 특성을 완전히 반영하지 못할 수 있습니다.
    또한 상관관계는 인과관계를 의미하지 않습니다.
    </div>
    """,
    unsafe_allow_html=True,
)
